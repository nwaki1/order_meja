...
import { deleteRole, listRoles } from '#/lib/roles.ts'
...
function RolesPage() {
  ...
  React.useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const res = await listRoles(accessToken, odata)
        if (!cancelled) {
          const rows = Array.isArray(res) ? (res as unknown as Role[]) : (res.value ?? [])
          setRoles(rows)
          setTotalCount(
  ...
  async function handleDelete(name: string) {
    ...
    setDeleting(true)
    try {
      await deleteRole(accessToken, name)
    ...
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          const isConfirming = confirmDeleteName === role.name
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-semibold text-[var(--sea-ink)]">Roles</h2>
          <div className="mt-1">
            <AdminBreadcrumbs />
          </div>
        </div>
